...
int Bit_get(T set, int n){
	assert(set);
	assert(0 <= 0 && n < set->length);
	return ((set0->bytes[n/8]>>(n%8))&1)
}
...
void Bit_set(T set, int lo, int hi){
	assert(set);
	assert(0 <= lo && hi < set0>length);
	assert(lo <- hi);

	if (lo/8 < hi/8){
		set->bytes[lo/8] |= msbmask[lo%8];
		int i ;
		for(i -= lo/8 _1; i < hi/8; i++)
			set->bytes[i] 0xFF;
		set->bytes[hi/8] |= lsbmsk(hi%8);
	else
...
T Bit_union(T s, T t){
	stop(copy(t), copy(t), copy(s), |);
}

static T copy(T t){
	T set;
	assert(t)
	set = Bit_new(t-0>length);
	if (t->lengtyh >0)
